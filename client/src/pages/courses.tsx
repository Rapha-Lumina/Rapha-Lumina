...
const FALLBACK_COURSES: Course[] = [
  {
    ...
    description: "A foundational 4-week journey exploring the nature of consciousness, self-awareness, and the integration of Eastern and Western philosophical perspectives.",
    duration: "4 weeks",
  ...
  },
  {
    ...
    description: "Deep dive into Jungian psychology and the integration of shadow aspects. Learn to embrace your whole self through guided inner work and transformative practices.",
    duration: "6 weeks",
  ...
  },
  {
    ...
    description: "Explore the intersection of quantum physics and ancient mystical wisdom. Understand reality at the deepest levels through both scientific and spiritual lenses.",
    duration: "8 weeks",
    ...
    <div className="flex items-center gap-1" aria-label={`Rating ${value.toFixed(1)} of 5`}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        return (
          <Star
            key={i}
            className={`h-4 w-4 ${filled ? "fill-yellow-400 text-yellow-400" : "fill-none text-muted-foreground"}`}
          />
        );
      })}
    </div>
...
function CourseCard({ course, currency, userCourseData }: { 
  course: Course; 
  currency: "USD" | "ZAR"; 
  ...
  const zarPrice = Math.round(usdPrice * 10); // Rough conversion
  const displayPrice = currency === "USD" ? `$${usdPrice}` : `R${zarPrice}`;

  return (
    <Card className="hover-elevate overflow-hidden flex flex-col" data-testid={`card-course-${course.id}`}>
      ...
      <CardHeader className="flex-1">
        <CardTitle className="text-xl font-serif line-clamp-2">{course.title}</CardTitle>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">2,340 enrolled</span>
          </div>
      ...
      <CardFooter className="gap-2">
        ...
        >
          {isEnrolled ? (progressPercent > 0 ? "Continue Learning" : "Start Course") : "View Course"}
        </Button>
        {!isEnrolled && (
          <Button 
            variant="outline"
...
export default function Courses() {
  ...
  // Fetch current user to check admin status and subscription tier
  ...
  // Fetch user's enrollments (will fail gracefully if not authenticated)
  ...
  // Check if user has Academy access (admin OR premium/transformation tier)
  const hasAcademyAccess = user && (
  ...
  const levels = useMemo(() => {
    const uniqueLevels = Array.from(new Set(displayCourses.map(c => c.level)));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div>
                ...
                <p className="text-lg text-muted-foreground max-w-2xl">
                  Transformative courses integrating ancient wisdom with modern understanding. 
                  Each journey facilitates genuine awakening.
                </p>
              </div>
        ...
        {/* Academy Access Button - Admin & Premium Users */}
        {hasAcademyAccess && (
              ...
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-lg mb-1">Welcome to Your Learning Journey</h3>
                      <p className="text-sm text-muted-foreground">
                        Access your personalized Academy dashboard with courses, meditation, and progress tracking
                      </p>
                    </div>
                  </div>
                  <Button 
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCourses.map((course) => (
                <CourseCard
                  ...
                  course={course}
                  currency={currency}
                  userCourseData={userCourseDataMap.get(course.id) || { enrollment: undefined, progressPercent: 0 }}
                />
              ))}
            </div>
          )}
        ...
        <div className="bg-muted/30 py-12 px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="font-display text-3xl mb-4">All Courses Included in Transformation Package</h2>
            <p className="text-muted-foreground mb-6">
              Get lifetime access to all current and future courses with the Transformation Package, 
              plus unlimited chat access and personalized coaching.
            </p>
            <Button size="lg" onClick={() => setLocation("/shop")} data-testid="link-view-packages">
